...
const ItemReportPost = ({item}) => {
    ...
    useEffect(() => {
        item?.report.forEach(item => {
            firestore().collection("users").doc(item).get().then(doc => {
                setUsersReport(usersReport => [...usersReport,doc.data()?.displayName])
                    })
                }
            )
    }, [])
    const handleOnRemovePost = () => {
        firestore().collection("postsUser").doc(item.id).delete()
        .then(() => {
        ...
        <View style={styles.itemReportPost}>
            <View style={styles.itemReportPostHeader}>
               { usersReport?.length == 1&&
                <Text style={styles.nameUserReport}>{usersReport[0]|| "Ai đó"} </Text>}
                { usersReport?.length > 1&&
                <Text style={styles.nameUserReport}>{usersReport[0]|| "Ai đó" } và {usersReport.length -1} người khác </Text>}
                <Text style={{fontSize:16}}>đã báo cáo bài viết này.</Text>
            </View>
            <ContentPost item={item}/>
            <View style={styles.btnChoice}>
                <TouchableOpacity style={styles.btnChoiceItem}
                    onPress={() => {
