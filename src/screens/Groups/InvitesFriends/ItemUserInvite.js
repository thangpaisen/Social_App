...
const ItemUserInvite = ({idUser,idGroup}) => {
    ...
    useEffect(() => {
        firestore().collection('users').doc(idUser).get().then(doc => {
    ...
    const handleOnInvite = () => {
        ...
        firestore().collection('users').doc(user.id).collection('inviteGroup').add({
            idGroup: idGroup,
            idUserInvite: auth().currentUser.uid,
            createdAt: new Date().getTime(),
        })
    ...
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity style={[styles.btnAdd,isInvite && styles.btnAddActive]} 
